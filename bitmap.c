...
int read_bmp(const char* file_name, uint8_t* p, int* w, int* h)
{
	...
	if (fp == NULL)
		return OPEN_FAILED;
	fread(&fh, 1, sizeof(BITMAP_FILEHEADER), fp);
	if (fh.Signature != BITMAP_SIGNATURE)
	{
		fclose(fp);
		return TYPE_MISMATCH;
	}
	fread(&dibh, 1, sizeof(BITMAPINFOHEADER), fp);
	...
	fseek(fp, sizeof(BITMAP_FILEHEADER) + fh.BitsOffset, SEEK_SET);
	if (dibh.Height > 0)
	{
...
int write_bmp(const char* file_name, uint8_t* p, int w, int h)
{
	...
	fwrite(&fh, 1, sizeof(BITMAP_FILEHEADER), fp);
	dibh.Width = w;
	dibh.Height = h;
	dibh.BitCount = 8;
	dibh.Compression = 0;
	...
	dibh.ClrUsed = 0;
	dibh.ClrImportant = 0;
	fwrite(&dibh, 1, sizeof(BITMAPINFOHEADER), fp);
	BGRA Palette[256];
	for (int i = 0; i < 256; i++)
	{
