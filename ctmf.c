        ...
        const int r, const int pad_left, const int pad_right)
{
    ...
    Histogram H;
    uint16_t *h_coarse, *h_fine, luc[16];
...
#if defined(__SSE2__) || defined(__MMX__)
    h_coarse = (uint16_t*) _mm_malloc(  1 * 16 * n * sizeof(uint16_t), 16 );
    h_fine   = (uint16_t*) _mm_malloc( 16 * 16 * n * sizeof(uint16_t), 16 );
    memset( h_coarse, 0,  1 * 16 * n * sizeof(uint16_t) );
    memset( h_fine,   0, 16 * 16 * n * sizeof(uint16_t) );
#else
    h_coarse = (uint16_t*) calloc(  1 * 16 * n, sizeof(uint16_t) );
    h_fine   = (uint16_t*) calloc( 16 * 16 * n, sizeof(uint16_t) );
#endif
    ...
    for ( i = 0; i < m; ++i ) {
        ...
        if ( pad_left ) {
             histogram_muladd( r, h_coarse, H.coarse );
        }
        ...
        for ( k = 0; k < 16; ++k ) {
            histogram_muladd( 2*r+1, &h_fine[16*n*k], &H.fine[k][0] );
        }
        ...
        for ( j = pad_left ? 0 : r; j < (pad_right ? n : n-r); ++j ) {
            ...
            histogram_add( &h_coarse[16*(MIN(j+r,n-1))], H.coarse );
            ...
            if ( luc[k] <= j-r ) {
                ...
                for ( luc[k] = j-r; luc[k] < MIN(j+r+1,n); ++luc[k] ) {
                    histogram_add( &h_fine[16*(n*k+luc[k])], H.fine[k] );
                }
                if ( luc[k] < j+r+1 ) {
                ...
                for ( ; luc[k] < j+r+1; ++luc[k] ) {
                    histogram_sub( &h_fine[16*(n*k+MAX(luc[k]-2*r-1,0))], H.fine[k] );
                    histogram_add( &h_fine[16*(n*k+MIN(luc[k],n-1))], H.fine[k] );
                }
            }
...
void ctmf(
        ...
        const int width, const int height,
        const int src_step, const int dst_step,
        const int r, const long unsigned int memsize
        )
{
    /*
    ...
    for ( i = 0; i < width; i += stripe_size - 2*r ) {
        ...
        ctmf_helper( src + i, dst + i, stripe, height, src_step, dst_step, r, i == 0, stripe == width - i );
